...
public class RestaurantDao {	
	...
	private final String IMG_URL_TABLE = "img_url";
	private final String PRICE_TABLE = "price";
	//CRUD常用操作的SQL语句模板
	...
	public Restaurant getRestaurantById(String argId){
		...
		try {
			...
			if(set.next()){
				ret = new Restaurant(set.getString(ID_TABEL),set.getString(RESTAURANT_NAME_TABLE),set.getString(RESTAURANT_ADDRESS_TABLE),set.getString(RESTAURANT_PHONE_TABLE),set.getString(IMG_URL_TABLE),set.getDouble(PRICE_TABLE));
			}
	...
	public Boolean addRestaurant(Restaurant value){
		...
		try {
			...
			ps.setString(5, value.getImgUrl());
			ps.setDouble(6, value.getPrice());
			int result  = -1;
